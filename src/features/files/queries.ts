...
export function useUploadFiles(teamId: string, orgId: string) {
  ...
  return useMutation({
    mutationFn: (files: File[]) =>
      Promise.all(files.map((file) => filesApi.upload(teamId, orgId, file))),
    onSuccess: (uploaded) => {
...
export function useDeleteFile(teamId: string) {
  const client = useQueryClient();
  return useMutation({
    mutationFn: (fileId: string) => filesApi.remove(teamId, fileId),
    onSuccess: () => {
      client.invalidateQueries({ queryKey: qk.files(teamId) });
